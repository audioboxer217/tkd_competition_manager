...
class TestPageRoutes:
    ...
    def test_admin_schedule_poomsae_before_kyorugi(self, client):
        """All poomsae divisions should appear before all kyorugi, regardless of sequence."""
        ring_id = _create_ring(client, "Ring 1").get_json()["id"]

        # Kyorugi at sequence 1
        ...
        kyorugi_match = Match.query.filter_by(division_id=kyorugi_id).first()
        client.put(f"/matches/{kyorugi_match.id}/schedule", data={"ring_sequence": "1"})

        # Poomsae at sequence 2 (higher than kyorugi — but still must come first)
        ...
        poomsae_match = Match.query.filter_by(division_id=poomsae_id).first()
        client.put(f"/matches/{poomsae_match.id}/schedule", data={"ring_sequence": "2"})

        resp = client.get("/admin/schedule")
        assert resp.status_code == 200
        html = resp.data.decode()
        # Poomsae Div (seq 2) should still appear before Kyorugi Div (seq 1)
    ...
    def test_ui_results_divisions_search_by_name_filters(self, client):
        ...
        resp = client.get("/ui/results_divisions?event_type=kyorugi&search=alice")
        assert resp.status_code == 200
        assert b"Alpha Div" in resp.data
        assert b"Beta Div" not in resp.data

    def test_ui_results_divisions_search_case_insensitive(self, client):
        div_id = _create_division(client, "Case Div", "kyorugi").get_json()["id"]
        _add_competitors(client, div_id, ["Charlie Brown"])

        resp = client.get("/ui/results_divisions?event_type=kyorugi&search=CHARLIE")
        assert resp.status_code == 200
    ...
    def test_ui_results_divisions_search_no_match_shows_empty(self, client):
        ...
        resp = client.get("/ui/results_divisions?event_type=kyorugi&search=zzznomatch")
        assert resp.status_code == 200
        assert b"No divisions found" in resp.data
...
class TestApiResponseHelpers:
    ...
    def test_success_response_custom_status(self, app):
        with app.test_request_context():
            resp, code = success_response({"id": 1}, status_code=201)
            assert code == 201
            assert resp.get_json()["data"] == {"id": 1}

    def test_error_response_structure(self, app):
        with app.test_request_context():
            resp, code = error_response("NOT_FOUND", "Item not found.", details={"id": 9}, status_code=404)
            assert code == 404
            body = resp.get_json()
            assert body["data"] is None
            assert body["error"]["code"] == "NOT_FOUND"
            assert body["error"]["message"] == "Item not found."
            assert body["error"]["details"] == {"id": 9}

    def test_error_response_empty_details_default(self, app):
        with app.test_request_context():
            resp, _ = error_response("BAD_REQUEST", "Oops.")
            assert resp.get_json()["error"]["details"] == {}


# ---------------------------------------------------------------------------
# API v1 — authentication enforcement
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# API v1 — JSON Content-Type enforcement
# ---------------------------------------------------------------------------
...
class TestApiV1Divisions:
    ...
    def test_create_division_kyorugi(self, api_client):
        ...
        body = resp.get_json()
        assert body["error"] is None
        assert body["data"]["name"] == "Male - Black Belt"
...
# ---------------------------------------------------------------------------
# API v1 — /api/v1/divisions/<id>/bracket
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# API v1 — /api/v1/matches/<id>/result
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# API v1 — legacy routes remain untouched
# ---------------------------------------------------------------------------


class TestLegacyRoutesUntouched:
    """Verify that legacy (non-API) routes continue to work as before."""

    def test_legacy_get_rings(self, client):
        client.post("/rings", json={"name": "R1"})
        resp = client.get("/rings")
        assert resp.status_code == 200
        data = resp.get_json()
        # Legacy format: list of objects, NOT the {"data": ..., "error": ...} envelope
        assert isinstance(data, list)
        assert data[0]["name"] == "R1"

    def test_legacy_get_divisions(self, client):
        client.post("/divisions", json={"name": "D1"})
        resp = client.get("/divisions")
        assert resp.status_code == 200
        data = resp.get_json()
        assert isinstance(data, list)
    ...
    def test_legacy_create_ring_response_format(self, client):
        resp = client.post("/rings", json={"name": "Legacy Ring"})
        assert resp.status_code == 201
        body = resp.get_json()
